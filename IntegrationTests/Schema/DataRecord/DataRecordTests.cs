...
public sealed class DataRecordTests : ApiTestBase
{
    ...
    public async Task Create_ValidRequest_ReturnsResponse()
    {
        ...
        var request = new CreateDataRecordRequest(metaTableId, 5);

        var response = await DataRecordClient.CreateAsync(request);
    ...
    public async Task GetAll_WithMetaTableIdFilter_ReturnsOnlyMatchingRecords()
    {
        ...
        var page = await httpResponse.Content.ReadFromJsonAsync<PageResponse<DataRecordResponse>>(
            ClientJson.Options);
